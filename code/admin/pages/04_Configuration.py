...
def validate_answering_prompt():
    """
    Validates the answering prompt to ensure it contains the required variables.
    """
    if "{sources}" not in st.session_state.answering_prompt:
        st.warning(
            "Your answering prompt doesn't contain the variable `{sources}`")
...
def validate_post_answering_prompt():
    """
    Validates the post answering prompt to ensure it contains the required variables.
...
try:
    with st.expander("Orchestrator configuration", expanded=True):
        cols = st.columns([2, 4])
        with cols[0]:
            st.selectbox("Orchestrator strategy", key='orchestrator_strategy',
    ...
    with st.expander("Prompt configuration", expanded=True):
        st.text_area("Answering prompt", key='answering_prompt',
                    on_change=validate_answering_prompt, help=answering_prompt_help, height=400)

        st.text_area("Post-answering prompt", key='post_answering_prompt',
                    on_change=validate_post_answering_prompt, help=post_answering_prompt_help, height=200)
        st.checkbox('Enable post-answering prompt',
                    key='enable_post_answering_prompt')
        st.text_area("Post-answering filter message", key='post_answering_filter_message',
                    help=post_answering_filter_help, height=200)

        st.checkbox('Enable Azure AI Content Safety',
    ...
    with st.expander("Logging configuration", expanded=True):
        st.checkbox('Log user input and output (questions, answers, chat history, sources)',
