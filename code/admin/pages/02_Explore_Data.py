...
st.set_page_config(page_title="Explore Data", page_icon=os.path.join(
...
try:
    ...
    unique_files = [filename['value']
                    for filename in results.get_facets()["title"]]
    ...
    data = [[json.loads(result['metadata'])['chunk'], result['content']]
            for result in results]
    df = pd.DataFrame(data, columns=('Chunk', 'Content')
                      ).sort_values(by=['Chunk'])
    st.table(df)
